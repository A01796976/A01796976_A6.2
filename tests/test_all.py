...
class TestAllSystem(unittest.TestCase):
    ...
    def test_hotel_full_lifecycle(self):
        ...
        self.assertFalse(self.hotel.create_hotel(1, "Duplicado", "MX", 10)) # Fallo intencional
        
        # 2. Mostrar
        info = self.hotel.display_hotel_info(1)
        self.assertEqual(info['name'], "Hotel Test")
        self.assertIsNone(self.hotel.display_hotel_info(999)) # No existe

        # 3. Modificar
        self.assertTrue(self.hotel.modify_hotel_info(1, name="Hotel Mod", location="US"))
        self.assertEqual(self.hotel.display_hotel_info(1)['location'], "US")
        self.assertFalse(self.hotel.modify_hotel_info(999, name="Fantasma")) # No existe
    ...
    def test_customer_full_lifecycle(self):
        ...
        # 2. Mostrar
        self.assertEqual(self.customer.display_customer_info(10)['name'], "Ana")
        self.assertIsNone(self.customer.display_customer_info(999))

        # 3. Modificar
        self.assertTrue(self.customer.modify_customer_info(10, email="b@b.com"))
        self.assertEqual(self.customer.display_customer_info(10)['email'], "b@b.com")
        self.assertFalse(self.customer.modify_customer_info(999, name="Nadie"))
    ...
    def test_customer_file_errors(self):
        with open(self.cust_file, "w") as f:
            f.write("basura")
    ...
    def test_reservation_lifecycle(self):
        ...
        self.assertFalse(self.reservation.create_reservation("RES2", 999, 1)) # Cliente no existe
        self.assertFalse(self.reservation.create_reservation("RES3", 100, 999)) # Hotel no existe

        # 4. Cancelar
        self.assertTrue(self.reservation.cancel_reservation("RES1"))
        self.assertFalse(self.reservation.cancel_reservation("RES1")) # Ya borrada
